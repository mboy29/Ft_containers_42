...
# include "../Iterators/Set_iterator.hpp" //  Tree iterator
# include "./Set_node.hpp" //  Tree node
	...
	template <class Value, class Compare, class Alloc = std::allocator<Value> >
		class set_tree {
			...
			public:
				...
				typedef ft::set_tree<value_type, Compare, Alloc>							tree_type;
				...
				typename Alloc::template rebind<ft::set_node<value_type, Alloc> >::other 	rebind_allocator_type;
			...
			public:
				...
				iterator		lower_bound(value_type const &val){
					...
					if (!(tmp = _search(val ,this->_root, &ptr)))
				...
				//  Bound :
				node_type	*bound(value_type const &val) const {
					node_type rhs = this->_root, lhs = rhs;
					
					while (rhs) {
						if (_comp(val, rhs->value)) {
							lhs = rhs;
							rhs = rhs->left;
						}
						else if (_comp(rhs->value, val))
							rhs = rhs->right;
						else 
							return (rhs);
					}
			...
			private :
				...
				//  Find :
				node_type	*_search(value_type idx, node_type *root, node_type **ptr) const {
					if (root) {
						if (_comp(idx, root->value)) {   
							*ptr = root;
							return (_search(idx, root->left, ptr));
						}
						else if (_comp(root->value, idx))
